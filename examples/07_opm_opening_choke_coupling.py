...
# -*- coding: utf-8 -*-
"""
co2_restart_coupled_coolprop_manywells_THPVALVE.py

Restart-based OPM Flow coupling using the stronger CoolProp + ManyWells-inspired
CO2 wellbore calculator.

This file keeps the robust idea from the previous RATE/WTEMP orchestrator, but it
adds pressure-control modes that are closer to the MSc LedaFlow/Eclipse workflow:

    1) fixed_rate
       OPM still receives RATE; external wellbore reconstructs the required THP
       and BHT. This is the old robust baseline, now using CoolProp properties.

    2) thp_limit
       RATE is no longer absolute truth. The script tries the demanded rate, but
       if the external required THP exceeds --thp-target-bar, it reruns the same
       OPM restart segment with a lower RATE until the THP constraint is satisfied.

Why not direct THP inside OPM?
------------------------------
For CO2STORE/THERMAL decks, direct THP+VFPINJ can be fragile. This orchestrator
therefore uses OPM RATE segments as robust residual evaluations while the external
wellbore/valve module controls the physically feasible rate.

Required files in the same folder / PYTHONPATH:
    Base.DATA
    co2_wellbore_coolprop_manywells.py
    this script

Example, old robust baseline:
    python3 co2_restart_coupled_coolprop_manywells_THPVALVE.py \
        --base-dir /path/to/BaseCase \
        --deck Base.DATA \
        --run-flow \
        --pressure-control-mode fixed_rate \
        --n-steps 5 --rate-schedule 100000 100000 100000 100000 100000

Example, pressure-limited coupling:
    python3 co2_restart_coupled_coolprop_manywells_THPVALVE.py \
        --base-dir /path/to/BaseCase \
        --deck Base.DATA \
        --run-flow \
        --pressure-control-mode thp_limit \
        --thp-target-bar 80 \
        --q-min-sm3-day 0 --q-max-sm3-day 150000 \
        --max-coupling-iter 8
"""
...
# The stronger calculator should be in the same directory as this script or in PYTHONPATH.
try:
    ...
    raise RuntimeError(
        "Cannot import co2_wellbore_coolprop_manywells.py. "
        "Put it next to this orchestrator or install it into PYTHONPATH. "
        "Also install runtime dependencies: pip install CoolProp numpy pandas."
    ) from exc
...
SECTION_NAMES = ["RUNSPEC", "GRID", "EDIT", "PROPS", "REGIONS", "SOLUTION", "SUMMARY", "SCHEDULE", "END"]
KNOWN_SCHEDULE_KEYWORDS = [
    "DATES", "TIME", "TSTEP", "RPTRST", "RPTSCHED", "SKIPREST", "INCLUDE",
    "WELLSTRE", "WELSPECS", "COMPDAT", "WINJGAS", "WCONINJE", "WCONPROD",
    "WTEMP", "WELOPEN", "WELTARG", "WELCNTL", "VFPINJ", "END",
]
...
def build_solution_section(original_solution: str, restart: RestartPointer) -> str:
    ...
    return (
        "SOLUTION\n"
        "-- Restart from previous accepted segment.\n"
        "RESTART\n"
        f" '{restart.root}' {int(restart.report_step)} 1* 1* /\n\n"
    )
...
def build_schedule_section(
    ...
    if is_restart:
        lines.append("-- Restart segment: do not replay old schedule before RESTART point.")
        lines.append("SKIPREST")
        lines.append("")
        # IMPORTANT for OPM Flow:
        # With SKIPREST, the restarted deck must still contain report steps
        # that hit the exact RESTART calendar time.  These alignment TSTEPs
        # are skipped, and the new controls below are applied after the restart time.
        align = list(control.restart_alignment_tsteps or [])
        if align:
            lines.append("-- Alignment report steps up to the RESTART time; skipped by SKIPREST")
            lines.append("TSTEP")
            lines.append(" " + " ".join(f"{float(x):.10g}" for x in align) + " /")
            lines.append("/")
            lines.append("")
    ...
    if not is_restart and static_well_setup.strip():
        lines.append("-- Static well definitions copied from Base.DATA")
        lines.append(static_well_setup.strip())
        lines.append("")

    if producer_control.strip():
        lines.append("-- Keep base producer/brine-control active")
    ...
    lines.append("WTEMP")
    lines.append(f" {well_name} {control.wtemp_used_C:.8g} /")
    lines.append("/")
    lines.append("")
    ...
    lines.append(f" {well_name} GAS OPEN RATE {control.rate_target_sm3_day:.8g} 1* {control.bhp_limit_bar:.8g} /")
    lines.append("/")
    lines.append("")

    lines.append("TSTEP")
    lines.append(f" {control.step_days:.10g} /")
    lines.append("/")
    lines.append("")
    lines.append("END")
    lines.append("")
...
# -----------------------------------------------------------------------------
# Restart artifacts and OPM run/read
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Professional coupling iteration report
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Phase split and figure generation
# -----------------------------------------------------------------------------
    ...
    for k in range(1, int(args.n_steps) + 1):
        ...
        for it in range(1, int(args.max_coupling_iter) + 1):
            ...
            if restart.root is not None:
                restart_alignment_tsteps = [float(tsteps[j] if j < len(tsteps) else args.step_days) for j in range(k - 1)]
